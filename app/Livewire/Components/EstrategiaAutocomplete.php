...
class EstrategiaAutocomplete extends Component
{
    public string $search = '';
    public ?int $selectedId = null;
    public string $selectedTitle = '';
    public bool $showDropdown = false;
    public string $placeholder = 'Buscar estrategia...';
    public string $name = 'estrategia_search';
    ...
    public function mount(?int $selectedId = null, string $placeholder = '')
    {
        ...
        if ($this->selectedId) {
            $estrategia = Strategy::find($this->selectedId);
            
            if ($estrategia) {
                $this->selectedTitle = strip_tags($estrategia->titulo);
    ...
    public function selectItem($id, $title)
    {
        ...
        $this->dispatch('estrategia-selected', id: $id, title: $title);
    }
    ...
    public function clearSelection()
    {
        ...
        // Dispatch event to parent component
        $this->dispatch('estrategia-selected', id: null, title: '');
    }
    ...
    public function results()
    {
        ...
        return Strategy::where('titulo', 'like', '%' . $this->search . '%')
            ->orWhere('subtitulo', 'like', '%' . $this->search . '%')
            ->select('id', 'titulo', 'subtitulo', 'status')
            ->orderBy('titulo')
            ->limit(10)
            ->get()
            ->map(function ($item) {
                return [
                    ...
                    'subtitulo' => strip_tags($item->subtitulo ?? ''),
                    'status' => $item->status
                ];
            });
    }
    ...
    {
        return view('livewire.components.estrategia-autocomplete');
    }
}