...
<flux:with-field :$attributes>
    ...
    >
        ...
        <dialog wire:ignore class="max-sm:max-h-full! rounded-xl shadow-xl sm:shadow-2xs max-sm:fixed! max-sm:inset-0! sm:backdrop:bg-transparent bg-white sm:border border-zinc-200">
            <ui-calendar class="isolate relative grid sm:grid-cols-[auto_1fr] grid-rows-[auto_auto_auto_auto_auto]" wire:ignore>
                <?php if ($presets): ?>
                    <ui-calendar-presets class="row-span-full max-sm:hidden border-e border-zinc-200">
                        <ui-radio-group class="flex flex-col gap-1 p-2 min-w-[120px]">
                            @foreach ($presetArray as $preset)
                                <ui-radio
                                    value="{{ $preset->value }}"
                                    class="text-sm font-medium text-zinc-600 data-checked:bg-(--color-accent) data-checked:text-(--color-accent-foreground) px-2 py-1.5 whitespace-nowrap rounded-lg hover:bg-zinc-100 hover:text-zinc-800"
                ...
                <?php if ($withInputs): ?>
                    <ui-calendar-inputs class="flex items-center p-2 border-b border-zinc-200">
                        <?php if ($range): ?>
                            <div class="sm:px-2 flex items-center gap-4">
                                <div class="flex items-center gap-2"><span class="max-sm:hidden text-sm font-medium text-zinc-800">Start</span> <flux:input type="date" class="w-[full] sm:w-[11.25rem]" /></div>
                                <div class="flex items-center gap-2"><span class="max-sm:hidden text-sm font-medium text-zinc-800">End</span> <flux:input type="date" class="w-[full] sm:w-[11.25rem]" /></div>
                            </div>
                        <?php else: ?>
                    ...
                    <div class="z-10 absolute top-0 inset-x-0 p-2">
                        <header class="flex justify-between items-center">
                            <div class="flex items-center gap-2">
                                <?php if ($selectableHeader): ?>
                                    ...
                                    <ui-calendar-year class="font-medium text-sm text-zinc-800 dark:text-white">
                                        <select
                                            class="h-10 py-0 border-0 text-sm sm:h-8 appearance-none rounded-lg bg-zinc-100 px-3 sm:ps-2 [background-position:_right_.25rem_center_!important] rtl:[background-position:_left_.25rem_center_!important] pe-[1.35rem] bg-[length:16px_16px] bg-[url('data:image/svg+xml,%3Csvg%20xmlns=%22http://www.w3.org/2000/svg%22%20viewBox=%220%200%2016%2016%22%20fill=%22%2300000040%22%20class=%22size-4%22%3E%3Cpath%20fill-rule=%22evenodd%22%20d=%22M4.22%206.22a.75.75%200%200%201%201.06%200L8%208.94l2.72-2.72a.75.75%200%201%201%201.06%201.06l-3.25%203.25a.75.75%200%200%201-1.06%200L4.22%207.28a.75.75%200%200%201%200-1.06Z%22%20clip-rule=%22evenodd%22/%3E%3C/svg%3E')] hover:bg-[length:16px_16px] hover:bg-[url('data:image/svg+xml,%3Csvg%20xmlns=%22http://www.w3.org/2000/svg%22%20viewBox=%220%200%2016%2016%22%20fill=%22%231f2937%22%20class=%22size-4%22%3E%3Cpath%20fill-rule=%22evenodd%22%20d=%22M4.22%206.22a.75.75%200%200%201%201.06%200L8%208.94l2.72-2.72a.75.75%200%201%201%201.06%201.06l-3.25%203.25a.75.75%200%200%201-1.06%200L4.22%207.28a.75.75%200%200%201%200-1.06Z%22%20clip-rule=%22evenodd%22/%3E%3C/svg%3E')] bg-no-repeat"
                                        >
                                            <template>
                                                <option><slot></slot></option>
                                            </template>
                            ...
                            <div class="flex items-center">
                                <?php if ($withToday): ?>
                                    <ui-calendar-today class="size-10 sm:size-8 rounded-lg flex items-center justify-center text-zinc-400 hover:bg-zinc-100 hover:text-zinc-800 [&[disabled]]:opacity-50 [&[disabled]]:pointer-events-none" aria-label="Previous month">
                                        <div class="relative">
                                            <template name="today">
                                ...
                                <ui-calendar-previous class="size-10 sm:size-8 rounded-lg flex items-center justify-center text-zinc-400 hover:bg-zinc-100 hover:text-zinc-800 [&[disabled]]:opacity-50 [&[disabled]]:pointer-events-none" aria-label="Previous month">
                                ...
                                <ui-calendar-next class="size-10 sm:size-8 rounded-lg flex items-center justify-center text-zinc-400 hover:bg-zinc-100 hover:text-zinc-800 [&[disabled]]:opacity-50 [&[disabled]]:pointer-events-none [&[disabled]_&]:text-zinc-400" aria-label="Next month">
                        ...
                        <div>
                            <template name="heading">
                                <div class="@if ($selectableHeader) [[data-month]:first-of-type_&]:opacity-0 @endif mb-2 px-2 h-10 sm:h-8 flex items-center">
                                    <div class="font-medium text-sm text-zinc-800"><slot></slot></div>
                                </div>
                            </template>
                            ...
                            <table>
                                <thead>
                                    <tr class="flex w-full">
                                        <?php if ($weekNumbers): ?>
                                            <th scope="col" class="{{ $sizeClasses }} text-sm font-medium text-zinc-500 flex items-center"><div class="w-full">#</div></th>
                                        <?php endif; ?>

                                        <template name="weekday">
                                            <th scope="col" class="{{ $sizeClasses }} text-sm font-medium text-zinc-500 flex items-center"><div class="w-full"><slot></slot></div></th>
                                        </template>
                                    </tr>
                                </thead>
                                        ...
                                        <tr class="flex w-full not-first-of-type:mt-1 [&:first-of-type_td[data-in-range]:not([data-selected]):first-child]:rounded-s-none [&:last-of-type_td[data-in-range]:not([data-selected]):last-child]:rounded-e-none">
                                            ...
                                            <template name="day">
                                                <?php if ($attributes->has('static')): ?>
                                                    <td class="p-0 data-unavailable:line-through data-in-range:bg-zinc-100 data-start:rounded-s-lg data-end:rounded-e-lg data-end-preview:rounded-e-lg first-of-type:rounded-s-lg last-of-type:rounded-e-lg [&[data-selected]+[data-selected]]:rounded-s-none">
                                                        <div class="relative {{ $sizeClasses }} text-sm font-medium text-zinc-800 flex items-center justify-center rounded-lg [td[data-selected]:has(+td[data-selected])_&]:rounded-e-none [td[data-selected]+td[data-selected]_&]:rounded-s-none [td[data-selected]_&]:bg-[var(--color-accent)] [td[data-selected]_&]:text-[var(--color-accent-foreground)] [td[data-selected]_&[disabled]]:opacity-50 disabled:text-zinc-400 disabled:pointer-events-none disabled:cursor-default">
                                                            <div class="absolute inset-0 hidden [td[data-today]_&]:flex justify-center items-end"><div class="mb-1 size-1 rounded-full bg-zinc-800 [td[data-selected]_&]:bg-white"></div></div>
                                                            <slot></slot>
                                                        </div>
                                                    </td>
                                                <?php else: ?>
                                                    <td class="_max-sm:data-outside:opacity-0 p-0 data-unavailable:line-through data-in-range:bg-zinc-100 data-start:rounded-s-lg data-end:rounded-e-lg data-end-preview:rounded-e-lg first-of-type:rounded-s-lg last-of-type:rounded-e-lg [&[data-selected]+[data-selected]]:rounded-s-none [[data-in-range]:not([data-selected]):not([data-end-preview])+&[data-outside]]:bg-linear-to-r [&[data-outside]:has(+[data-in-range])]:bg-linear-to-l from-zinc-100 from-1% [&[data-outside]:has(+[data-in-range][data-selected])]:bg-none!">
                                                        <ui-tooltip position="top">
                                                            <button type="button" class="{{ $sizeClasses }} text-sm font-medium text-zinc-800 flex flex-col items-center justify-center rounded-lg hover:bg-zinc-800/5 [td[data-selected]:has(+td[data-selected])_&]:rounded-e-none [td[data-selected]+td[data-selected]_&]:rounded-s-none [td[data-selected]_&]:bg-[var(--color-accent)] [td[data-selected]_&]:text-[var(--color-accent-foreground)] [td[data-selected]_&[disabled]]:opacity-50 disabled:text-zinc-400 disabled:pointer-events-none disabled:cursor-default [[readonly]_&]:pointer-events-none [[readonly]_&]:cursor-default [[readonly]_&]:bg-transparent">
                                                                <div class="relative">
                                                                    <div class="absolute inset-x-0 bottom-[-3px] hidden [td[data-today]_&]:flex justify-center items-end"><div class="size-1 rounded-full bg-zinc-800 [td[data-selected]_&]:bg-white"></div></div>

                                                                    <div><slot></slot></div>

                                                                    <template name="subtext">
                                                                        <div class="absolute inset-x-0 bottom-[-1rem] flex justify-center font-medium text-xs text-zinc-400 [[data-date-variant='success']_&]:text-lime-600 [[data-date-variant='warning']_&]:text-yellow-600 [[data-date-variant='danger']_&]:text-rose-500">
                                                                            <slot></slot>
                                                                        </div>
                                                                    </template>
                                                                </div>
                                                            ...
                                                            <template name="details">
                                                                <div popover="manual" class="relative py-2 px-2.5 rounded-md text-xs text-white font-medium bg-zinc-800 p-0 overflow-visible">
                                                                    <slot></slot>
                                                                </div>
                                                            </template>
                                                        </ui-tooltip>
                ...
                <?php if ($presets): ?>
                    <ui-calendar-presets class="block sm:hidden px-4">
                        <select class="appearance-none w-full ps-3 pe-10 block h-10 py-2 text-sm rounded-lg shadow-2xs border bg-white  text-zinc-700  has-[option.placeholder:checked]:text-zinc-400  disabled:shadow-none border-zinc-200 border-b-zinc-300/80 " data-flux-control="" data-flux-select-native="" data-flux-group-target="">
                            <option value="" disabled="" selected="" class="placeholder">Elige un rango</option>
                            @foreach ($presetArray as $preset)
