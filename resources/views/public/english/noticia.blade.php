...
@section('title', ' - ' . strip_tags($noticia->titulo_en))
                                    ...
                                    <div class="absolute inset-0 transition-opacity duration-300 z-10"
                                        ...
                                        <div x-show="!isPlayingVideo" class="relative w-full h-full cursor-pointer z-10" @click="playVideo()">
                                            <img src="{{ $video->thumbnail_url }}"
                                                 alt="{{ strip_tags($noticia->titulo_en) }} - Video"
                                ...
                                <div class="relative w-full h-full">
                                    @foreach($noticia->multimedia as $index => $multimedia)
                                        ...
                                        <div class="absolute inset-0 transition-opacity duration-300 z-10"
                                             ...
                                             x-transition:leave-start="opacity-100"
                                             x-transition:leave-end="opacity-0">
                                            <img src="{{ asset('storage/' . $multimedia->filename) }}"
                                                 alt="{{ strip_tags($noticia->titulo_en) }} - Image {{ $index + 1 }}"
                    ...
                    @else
                        <div class="mb-6">
                            <img src="{{ asset('assets/images/casa/banner.jpg') }}"
                                 ...
                                 alt="{{ strip_tags($noticia->titulo_en) }}">
                        </div>
                    @endif
                </div>
                ...
                <div>
                    <div class="flex flex-col gap-2">
                        ...
                        <h1 class="!text-3xl !font-bold text-green-600 !tracking-tight mb-6 leading-tight">
                            {!! str_replace(['<p>', '</p>'], '', $noticia->titulo_en) !!}
                        </h1>

                        <!-- Description -->
                        @if($noticia->descripcion_en)
                            <div class="font-libre text-gray-700 text-lg leading-6 mb-6">
                                {{ $noticia->descripcion_en }}
                            </div>
                        @endif
                        ...
                        <div class="flex items-center gap-2 text-green-600 mb-2">
                            ...
                            <time class="!text-gray-600">{{ $noticia->fecha_publicacion->isoFormat('MMMM D, Y') }}</time>
                        </div>
                    ...
                    @if($noticia->contentImage)
                        <div class="flex justify-center mt-12">
                            <div class="w-full md:w-1/2">
                                <img src="{{ asset('storage/' . $noticia->contentImage->filename) }}"
                                     alt="{{ strip_tags($noticia->titulo_en) }}"
                        ...
                        <div class="flex flex-col rounded-xl overflow-hidden donation-section">
                            <div class="relative h-[450px] bg-green-600 overflow-hidden">
                                @if($noticia->donationMultimedia)
                                    <img src="{{ asset('storage/' . $noticia->donationMultimedia->filename) }}"
                                         alt="Support this cause"
                                         class="w-full h-full object-cover">
                            ...
                            <div class="donation-content">
                                {!! $noticia->donation_content_en ?: $noticia->donation_content !!}
                            </div>
                        </div>

                        <!-- Right side - Donation Form -->
                        <livewire:components.donation-form :noticia="$noticia" :language="'en'" />
                    </div>
                </div>
            </div>
        </section>
        ...
        <section class="py-20 bg-gray-50">
            <div class="container mx-auto">
                ...
                <div class="text-center border-b-2 border-green-600 pb-2 mb-8">
                    <span class="bg-green-600 px-8 py-2 text-white font-serif text-2xl leading-4">Related news</span>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    @foreach($relacionadas as $relacionada)
                        <div class="">
                            <a href="{{ route('english.noticia', $relacionada->slug_en ?? $relacionada->slug) }}" class="block text-center hover:opacity-80 transition-opacity duration-300 !no-underline hover:!no-underline">
                                <div class="mb-3 h-64">
                                    @if($relacionada->featured_image)
                                        <img src="{{ asset('storage/' . $relacionada->featured_image->filename) }}"
                                             alt="{{ strip_tags($relacionada->titulo_en) }}"
                                ...
                                <div class="text-left">
                                    ...
                                    <h3 class="font-sans text-green-600 !text-xl leading-6 tracking-tight mb-2">
                                        {{ strip_tags($relacionada->titulo_en ?: $relacionada->titulo) }}
                                    </h3>
                                    <!-- Excerpt -->
                                    <p class="text-gray-600 text-sm mb-2">{{ $relacionada->descripcion_en ?: ($relacionada->descripcion ?: Str::limit(strip_tags($relacionada->contenido_en ?: $relacionada->contenido), 150)) }}</p>
                                </div>
                            </a>
                        </div>
                    @endforeach
                    ...
                    <div class="row">
                        <div class="col-lg-7">
                            <div class="donacion-imagen">
                                @if($noticia->donationMultimedia)
                                    <img src="{{ asset('storage/' . $noticia->donationMultimedia->filename) }}" alt="Thank you very much!">
                                @elseif($noticia->featured_image)
                                    <img src="{{ asset('storage/' . $noticia->featured_image->filename) }}" alt="Thank you very much!">
                                @else
                                    <img src="{{ asset('assets/images/donaciones/modal-bg4.jpg') }}" alt="Thank you very much!">
                                @endif
                            </div>
                            ...
                            <div class="donacion-text-wrapper">
                                ...
                                <p>Your contribution to <strong>{{ strip_tags($noticia->titulo_en) }}</strong> <span>will help</span> us continue creating <span>social change</span> through <span>community and cultural processes</span> for the common good and the environment.</p>
                        ...
                        <div class="col-lg-5">
                            <div class="donacion-content">
                                ...
                                @if($noticia->donation_content_en || $noticia->donation_content)
