...
@section('content')
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div>
                    ...
                    @if($exposicion->multimedia && $exposicion->multimedia->count() > 0)
                        <div class="mb-6" x-data="{ currentImageIndex: 0, totalImages: {{ $exposicion->multimedia->count() }} }">
                ...
                <div>
                    ...
                    <div class="flex flex-col gap-4">
                        <h1 class="!text-3xl !font-bold text-green-600 !tracking-tight mb-6 leading-tight">{!! str_replace(['<p>', '</p>'], '', $exposicion->titulo) !!}</h1>
                ...
                @if($exposicion->programas && $exposicion->programas->where('assign_to_expo_proyecto', true)->count())
                    <div class="col-span-2 mt-8">
                        <h2 class="text-2xl font-bold text-green-600 mb-6">Related Activities</h2>
                        <div class="space-y-8">
                            @foreach($exposicion->programas->where('assign_to_expo_proyecto', true) as $programa)
                                    ...
                                    <div class="col-span-4">
                                        @if($programa->multimedia && $programa->multimedia->count() > 0)
                                            ...
                                            @if($programa->multimedia->count() > 1)
                                                <div x-data="{ currentImageIndex: 0, totalImages: {{ $programa->multimedia->count() }} }" class="relative">
                                                        ...
                                                        <div class="relative w-full h-full">
                                                            @foreach($programa->multimedia as $index => $multimedia)
                                                                <div class="absolute inset-0 transition-opacity duration-300"
                                                                     ...
                                                                     x-transition:leave-end="opacity-0">
                                                                    <img src="{{ $multimedia->multimedia?->url }}"
                                                                         alt="{{ strip_tags($programa->titulo) }} - Image {{ $index + 1 }}"
                                                            ...
                                                            <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
                                                                @foreach($programa->multimedia as $index => $multimedia)
                                                                    <button @click="currentImageIndex = {{ $index }}"
                                                                            ...
                                                                            :class="currentImageIndex === {{ $index }} ? 'bg-white' : 'bg-white/60'">
                                                                    </button>
                                                                @endforeach
                                                            </div>
                                                        @endif
                                    ...
                                    <div class="col-span-8">
                                        ...
                                        @if($programa->contenido)
                                            <div class="text-sm text-gray-600 leading-relaxed mb-4">
                                                {!! Str::limit(strip_tags($programa->contenido), 200) !!}
                                            </div>
                                        @endif
                                        
                                        {{-- Date --}}
                                        @if($programa->fecha)
                                            <p class="text-sm text-gray-500 mb-3">{{ $programa->fecha }}</p>
                                        @endif
                                        
                                        {{-- Categories --}}
                                        @if($programa->tags && $programa->tags->count())
                                            <div class="flex flex-wrap gap-2 mb-4">
                                                @foreach($programa->tags as $tag)
                                        ...
                                        @if($programa->titulo && strip_tags($programa->titulo))
                                            <a href="{{ route('english.programa.estrategia-' . ($programa->tipo === 'local' ? 'local' : 'externa') . '.detalle', ['slug' => \Illuminate\Support\Str::slug(strip_tags($programa->titulo))]) }}" 
                                        ...
                                        <div x-data="{ show: true }" x-show="show" class="bg-red-50 border border-red-200 rounded-lg p-4">
                                            <div class="flex items-start justify-between">
                                                <div class="flex items-start">
                                                    ...
                                                    <div>
                                                        <h4 class="text-sm font-semibold text-red-800 mb-1">File Not Available</h4>
                                                        <p class="text-sm text-red-700">
                                                            The requested file is not available for download at this time. 
                ...
                @if($exposicion->publicaciones && $exposicion->publicaciones->count())
                    <div class="space-y-8">@foreach($exposicion->publicaciones as $publicacion)
                            <div class="grid grid-cols-12 gap-6 items-start">
                                ...
                                <div class="col-span-4">
                                    <div class="mb-4">
                                        <img src="{{ $publicacion->publicacion_thumbnail?->url }}" 
                                             alt="{{ strip_tags($publicacion->titulo) }}" 
                                             class="w-full h-auto rounded-lg shadow-md">
                                    </div>
                                </div>
                                ...
                                <!-- Content column -->
                                <div class="col-span-8">
                                    <h3 class="!text-lg !font-libre !font-bold !text-green-600 mb-2">{{ strip_tags($publicacion->titulo) }}</h3>
                ...
                @if($exposicion->videos && $exposicion->videos->count())
                    <div x-data="{
                        ...
                        <div x-show="!isPlaying" class="flex flex-col gap-4">
                            <!-- Thumbnail Carousel -->
                            <div class="relative h-52 w-full overflow-hidden group">
                                    ...
                                    @foreach($exposicion->videos as $index => $video)
                                        <div class="absolute inset-0 transition-opacity duration-300"
                                            ...
                                            <div class="relative w-full h-full cursor-pointer group/video"
                                                ...
                                                <div class="w-full h-full bg-gray-800 flex items-center justify-center rounded" style="display: none;">
                                                ...
                                                <!-- Play Button Overlay -->
                                                <div class="absolute inset-0 flex items-center justify-center transition-all duration-300 rounded bg-black/20 ">
                                                    <div class="bg-green-600 hover:bg-green-700 text-white rounded-full p-4 transform group-hover/video:scale-110 transition-all duration-300 shadow-lg">
                    ...
                    <div x-data="{ 
                        ...
                        <div class="relative w-full">
                            ...
                            <div class="relative w-full overflow-hidden group">
                                <!-- Noticias Container -->
                                <div class="relative w-full min-h-[120px]">
                                    @foreach($exposicion->noticias as $index => $noticia)
                                        <div class="absolute inset-0 transition-opacity duration-300"
                                            ...
                                            <a href="{{ route('noticia', $noticia->slug) }}" class="flex items-center gap-4 w-full hover:opacity-80 transition-opacity duration-300 !no-underline hover:!no-underline py-4">
                                                <!-- Image -->
                                                <div class="flex-shrink-0">
                                                    ...
                                                    @else
                                                        <div class="w-24 h-24 bg-gradient-to-br from-green-100 to-green-200 flex items-center justify-center">
                                                            <!-- Generic news icon -->
                                                ...
                                                <div class="flex-1 min-w-0">
                                                    ...
                                                    <!-- Date -->
                                                    <p class="text-gray-500 text-xs">{{ $noticia->fecha_publicacion->format('M d, Y') }}</p>
                                                </div>
                                            </a>
                                        </div>
                                    @endforeach
    ...
    <!-- PDF Viewer Modal -->
    <div x-data="{ showModal: false, archivoUrl: '' }" 
         x-show="showModal" 
         ...
         @ver-archivo.window="showModal = true; archivoUrl = $event.detail.url">
